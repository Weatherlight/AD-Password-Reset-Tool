...
# --- 初期設定（デフォルトのパス設定） ---
# スクリプトが保存されているフォルダを取得
$scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
if ([string]::IsNullOrEmpty($scriptDir)) { $scriptDir = Get-Location }

# デフォルトのログフォルダを「スクリプトと同じ場所\logs」に設定
$defaultLogFolder = Join-Path $scriptDir "logs"
$global:currentLogFolder = $defaultLogFolder

# --- メインウィンドウの設定 ---
$form = New-Object Windows.Forms.Form
$form.Text = "ADパスワード一括リセットツール (ログ選択機能付)"
$form.Size = New-Object Drawing.Size(520, 450) # ボタン追加のため少し高くしました
...
$txtLogOutput.Location = New-Object Drawing.Point(20, 200)
$txtLogOutput.Size = New-Object Drawing.Size(460, 150)
...
$btnRun.Location = New-Object Drawing.Point(190, 155)
...
$btnRun.Add_Click({
        if ([string]::IsNullOrWhiteSpace($txtFilePath.Text)) {
            [Windows.Forms.MessageBox]::Show("CSVファイルを選択してください。")
            return
        }
        ...
        if (!(Test-Path $global:currentLogFolder)) {
            try {
                ...
                [Windows.Forms.MessageBox]::Show("フォルダを作成できませんでした。権限を確認してください。")
                return
            }
        }
        ...
        $txtLogOutput.Clear()
        Write-GuiLog "処理を開始します..."
        Write-GuiLog "ログ出力先: $global:activeLogFile"
        ...
        try {
            ...
            Write-GuiLog "完了しました。"
            [Windows.Forms.MessageBox]::Show("すべての処理が完了しました。`nログファイル: $global:activeLogFile")
        }
        catch {
